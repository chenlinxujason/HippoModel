...
std::vector<std::string> orderedOutputGroups() {
    return {"CA3_PC", "CA3_AAC", "CA3_BC", "CA3_BCCCK", "CA3_BisC", "CA3_MFA", "CA3_OLM", "CA3_TL"};
}

SpikeMonitor* monitorFor(CA3NetworkContext& ctx, const std::string& group) {
    if (group == "CA3_PC") return ctx.mon_ca3_pc;
    if (group == "CA3_AAC") return ctx.mon_ca3_aac;
    if (group == "CA3_BC") return ctx.mon_ca3_bc;
    if (group == "CA3_BCCCK") return ctx.mon_ca3_bccck;
    if (group == "CA3_BisC") return ctx.mon_ca3_bisc;
    if (group == "CA3_MFA") return ctx.mon_ca3_mfa;
    if (group == "CA3_OLM") return ctx.mon_ca3_olm;
    if (group == "CA3_TL") return ctx.mon_ca3_tl;
    return nullptr;
}

}  // namespace

CA3Evaluator::CA3Evaluator(std::unique_ptr<CA3NetworkContext> context, OptimizationLogger* logger)
    : ctx_(std::move(context)), logger_(logger) {}

double CA3Evaluator::targetFor(const std::string& group) const {
...
std::pair<double, double> CA3Evaluator::boundsFor(const std::string& group) const {
...
void CA3Evaluator::measureFiringRates(const std::vector<double>& params, bool publish_ei) {
    scaleCA3Weights(*ctx_, params);
    ...
    std::cout << "[measureFRs] CA3_PC=" << mean_rates_["CA3_PC"] << " CA3_AAC=" << mean_rates_["CA3_AAC"] << " CA3_BC=" << mean_rates_["CA3_BC"] << " CA3_BCCCK=" << mean_rates_["CA3_BCCCK"] << " CA3_BisC=" << mean_rates_["CA3_BisC"] << " CA3_MFA=" << mean_rates_["CA3_MFA"] << " CA3_OLM=" << mean_rates_["CA3_OLM"] << " CA3_TL=" << mean_rates_["CA3_TL"] << std::endl;
}

double CA3Evaluator::objective(const std::vector<double>& params, bool output) {
    ...
    SpikeMonitor* mainMon = monitorFor(*ctx_, "CA3_PC");
    ...
    result.corr_mean_abs = dvSliceCorrMeanAbsZeroLagRandom(
        mainMon,
        ctx_->connectivity.loc_y.at("CA3_PC"),
    ...
    if (output) {
        std::cout << "CA3_PC corr=" << result.corr_mean_abs
