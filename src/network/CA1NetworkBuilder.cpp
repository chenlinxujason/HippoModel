...
std::map<std::string, double> scaleMapFromVector(const CA1RegionSpec& spec, const std::vector<double>& scaling_factors) {
...
void updateConnectionWeights(CA1NetworkContext& ctx, const std::map<std::string, double>& scale_by_name) {
...
void configureCA1ExternalInput(CA1NetworkContext& ctx, const std::string& group_name) {
    const auto src_it = ctx.config.inputs.sources.find(group_name);
    if (src_it == ctx.config.inputs.sources.end()) {
        throw std::runtime_error("Missing CA1 input source for group: " + group_name);
    }
    ...
    if (input.mode == "file") {
        ...
        throw std::runtime_error("Unsupported CA1 input mode for " + group_name + ": " + input.mode);
    }
}

}  // namespace

std::unique_ptr<CA1NetworkContext> buildCA1Network(const RuntimeConfig& cfg, const std::vector<double>& scaling_factors) {
    auto ctx = std::make_unique<CA1NetworkContext>();
    ctx->config = cfg;
    ctx->spec = makeCA1RegionSpec();
    ctx->connectivity = loadCA1Connectivity(cfg, ctx->spec);

    updateConnectionWeights(*ctx, scaleMapFromVector(ctx->spec, scaling_factors));

    ctx->sim = std::make_unique<CARLsim>("CA1_refactor", GPU_MODE, USER, cfg.simulation.num_gpus, cfg.simulation.seed);
    ...
    configureCA1ExternalInput(*ctx, "EC_LEC3");
    configureCA1ExternalInput(*ctx, "EC_MEC3");
    configureCA1ExternalInput(*ctx, "CA3_PC");

    configureCA1CellModels(*ctx->sim, ctx->group_ids);

    ctx->connection_generator = std::make_unique<CA1ConnectionGenerator>(ctx->spec, ctx->connectivity, ctx->connection_weights);
    ctx->sim->setUserConnectivity(ctx->connectivity.divergence);
    connectCA1Network(*ctx->sim, ctx->connection_generator.get(), ctx->group_ids, ctx->connection_ids);
    for (const auto& conn : ctx->spec.connections) {
    ...
    ctx->mon_ca1_pc = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_PC"), spikeOut);
    ctx->mon_ca1_aac = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_AAC"), spikeOut);
    ctx->mon_ca1_bc = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_BC"), spikeOut);
    ctx->mon_ca1_bccck = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_BCCCK"), spikeOut);
    ctx->mon_ca1_bisc = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_BisC"), spikeOut);
    ctx->mon_ca1_ng = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_NG"), spikeOut);
    ctx->mon_ca1_olm = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_OLM"), spikeOut);
    ctx->mon_ca1_tl = ctx->sim->setSpikeMonitor(ctx->group_ids.at("CA1_TL"), spikeOut);
