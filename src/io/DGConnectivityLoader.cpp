...
std::map<std::string, GroupSpec> groupSpecByName(const DGRegionSpec& spec) {
...
ConnectivityData loadDGConnectivity(const RuntimeConfig& cfg, const DGRegionSpec& spec) {
    ...
    for (const auto& group : spec.groups) {
        ...
        for (int id = range[0]; id <= range[1]; ++id) {
            ...
            if (group.name == "DG_GC") {
                if (locs_hc[id][0] < 0.0f) data.dg_gc_infra_global_ids.push_back(id);
                else data.dg_gc_supra_global_ids.push_back(id);
            }
        }
    }
    ...
    std::cout << "Number of IDs classified as supra: " << data.dg_gc_supra_global_ids.size() << std::endl;
    std::cout << "Number of IDs classified as infra: " << data.dg_gc_infra_global_ids.size() << std::endl;
    return data;
}
